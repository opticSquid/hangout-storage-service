...
func Download(workerId int, ctx context.Context, minioClient *minio.Client, file *files.File, cfg *koanf.Koanf, log logger.Log) {
	log.Info(ctx, "Downloading file", "file", file.Filename, "worker-id", workerId)
	err := minioClient.FGetObject(ctx, cfg.String("minio.upload-bucket"), file.Filename, "/tmp/"+file.Filename, minio.GetObjectOptions{})
	if err != nil {
		log.Error(ctx, "Error occured while downloading file", "file", file.Filename, "worker-id", workerId)
	}
}
...
func UploadDir(workerId int, ctx context.Context, minioClient *minio.Client, event *files.File, cfg *koanf.Koanf, log logger.Log) {
	...
	log.Info(ctx, "Starting to upload directory to Minio/s3", "directory", currentDir, "worker-id", workerId)
	// Walk through the folder and upload files
	err := filepath.Walk(currentDir, func(path string, info os.FileInfo, err error) error {
		if err != nil {
			log.Error(ctx, "Error occured while traversing through current file in the directory", "file", event.Filename, "error", err, "worker-id", workerId)
			return err
		}
		log.Debug(ctx, "trying to upload file", "file name", info.Name(), "path", path, "worker-id", workerId)
		// Skip directories
		if info.IsDir() {
			log.Debug(ctx, "A nested dirctory was encountered, skipping uploading the directory", "directory", info.Name(), "worker-id", workerId)
			return nil
		}
		...
		if contentType == "" {
			log.Debug(ctx, "Skipping uploading unsupported file type", "file", info.Name(), "worker-id", workerId)
			return nil
		}
		...
		if err != nil {
			log.Error(ctx, "could not open the file in the directory", "file", file.Name(), "error", err, "worker-id", workerId)
			return err
		}
		defer file.Close()
		log.Debug(ctx, "opened file for uploading", "file", file.Name(), "worker-id", workerId)
		// Upload the file
		objectName := baseFilename + "/" + info.Name()
		log.Debug(ctx, "printing upload params", "storage-bucket", cfg.String("minio.storage-bucket"), "object-name", objectName, "file-location", file.Name(), "worker-id", workerId)
		_, err = minioClient.FPutObject(ctx, cfg.String("minio.storage-bucket"), objectName, file.Name(), minio.PutObjectOptions{
			ContentType: contentType,
		})
		if err != nil {
			log.Error(ctx, "Failed to upload file", "file", file.Name(), "error", err, "worker-id", workerId)
			return err
		}
	...
	if err != nil {
		log.Error(ctx, "Error walking the folder", "folder", currentDir, "error", err, "worker-id", workerId)
	}
	log.Info(ctx, "Folder uploaded successfully", "directory", currentDir, "worker-id", workerId)
}
